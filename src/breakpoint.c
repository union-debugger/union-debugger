...
{
    errno = 0;
    u64 peek_data = ptrace(PTRACE_PEEKDATA, cfg->inferior_pid, address, NULL);
    if (errno != 0) {
        return -1;
    }
    ...
    if (ptrace(PTRACE_POKEDATA, cfg->inferior_pid, address, poke_data) < 0) {
        return -1;
    }
...
static
i32 breakpoint_enable(config_t* cfg, size_t const address)
{
...
static
i32 breakpoint_disable(config_t* cfg, size_t const address)
{
    if (cfg->state != STATE_RUNNING) {
        return -1;
    }
...
i32 breakpoint_new(config_t* cfg, size_t const address)
{
    ...
    printf("%sinfo:%s breakpoint #%zu set at address %zux\n", MAGENTA, NORMAL, cfg->breakpoints->len, address);
    return ret;
}
...
i32 breakpoint_remove_address(config_t* cfg, size_t const address)
{
    ...
    printf("%swarning:%s no breakpoint set at address %zu\n", YELLOW, NORMAL, address); 
    return -1;
}
...
i32 breakpoint_step(config_t* cfg)
{
    ...
    if (ptrace(PTRACE_SETREGS, cfg->inferior_pid, NULL, &registers) < 0) {
        return -1;
    }

    if (breakpoint_disable(cfg, b->address) < 0) {
        return -1;
    }
    if (ptrace(PTRACE_SINGLESTEP, cfg->inferior_pid, NULL, NULL) < 0) {
        return -1;
    }
    debugger_wait_signal(cfg);
    if (breakpoint_enable(cfg, b->address) < 0) {
        return -1;
    }
